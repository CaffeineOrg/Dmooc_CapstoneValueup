...
def text(request):  # STT 버튼 호출시 실행
    ...
    if request.method == 'POST':
        ...
        if lan == "en":
            try:  # STT
                text_all,time_stamp = transcribe_gcs_en(gcs_file, contents[-1], 44100)
            except:  # STT
                text_all,time_stamp = transcribe_gcs_en(gcs_file, contents[-1], 48000)
        else:
            try:  # STT
        ...
        # 웹으로 보낼 데이터
        gen = {
            'text_all': highlight_keywords(text_all),
        }
...
def keytext(request):  # 키워드 추출을 위한 메소드
    if request.method == 'POST':
        ...
        text_re = text_re.replace('''<span style="color:#ff58b0; text-decoration:underline; font-weight:bold;">''', '')
        text_re = text_re.replace('</span>', '')
        print(text_re)
...
def history_result(request, id):  # 게시판 결과물을 위한 메소드
    ...
    history_lecture.lecture_note = highlight_keywords(history_lecture.lecture_note)
...
def index_result(request, lecture_name):  # 게시판 결과물을 위한 메소드
    ...
    itemp = history_lecture[0]

    itemp.lecture_note = highlight_keywords(itemp.lecture_note)

    context = {'history_lecture': itemp}
